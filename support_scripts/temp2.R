...
HPAA <- function(HPA, fs, halftime, integration = c("Euler", "RK4")[1]){
  ...
  return(data.frame(HPA=c(HPA,0),C=C))
}
...
makeHPA <- function(day, fwake, fstress, settings,n){
  ...
  {set.seed(n); spikes <- abs(-rexp(fwake, rate = settings$nightgrowxp) + with(settings, waketime + CAR))
    ...
    spikes <- switch(settings$anticipation,
                     ...
                     YES = c(abs(-rexp(fwake+fstress*wordayfactor, rate = settings$nightgrowxp) + with(settings, waketime + CAR)),
                             runif(fstress*wordayfactor,min = settings$worktime[[1]], max = settings$worktime[[2]])))
  }
...
dosim <- function(settings, simperson, n, subDir){   
  ...
  ## calculate cortisol
  Qdata.pp <- HPAA(HPA, settings$fs, simperson$halftime_pp, settings$integration)
  Qdata.pp <- cbind(time=seq(0,24*settings$days,1/settings$fs),person = factor(rep(simperson$simperson,nrow(Qdata.pp))),Qdata.pp)
  return(Qdata.pp)
}
...
exesim <- function(paramset,defaults,subDir){
  ...
  Qdata <- (ldply(seq(settings$people), function(id) dosim(settings = settings, simperson = simpopulation[id,], n = n+id, subDir = subDir)))
