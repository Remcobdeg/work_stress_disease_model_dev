...
if (load_from_file == F){
  ...
  settings$stressfSD <- 12 #frequency of stress stressors
...
makeHPA <- function(day, simperson, settings,n){
  ...
  {set.seed(n); spikes <- abs(-rexp(simperson$wakefM_pp+1, rate = settings$nightgrowxp) + with(settings, waketime + CAR))
    ...
    spikes <- c(abs(-rexp(simperson$wakefM_pp+1, rate = settings$nightgrowxp) + with(settings, waketime + CAR)),
...
simlog <- list() #create a data frame for logging information about the simulation
