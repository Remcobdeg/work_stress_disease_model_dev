...
paramsets <- list( 
  list(#simulation 1
    ...
    worktime = c(8,19),
    dayoff = c(0,5,6), #from 0-6: 'Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'
    kHPA = 2.09675, #set to NA to have to code determine the scaling factor
    thAM = 400
  ),list(#simulation 3
    worktime = c(8,19),
    dayoff = c(0,5,6), #from 0-6: 'Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'
...
exesim <- function(paramset,defaults,subDir){
  ...
  if(is.na(settings$kHPA)){settings$kHPA<-9/mean(ldply(seq(settings$people), function(id) getscale(settings = settings, simperson = simpopulation[id,], n = n+id))[,1])}
