...
if (load_from_file == F){
  ...
  ## simulation settings
  settings$dt <- dt <- 1/6 #sampling time, in hours
  settings$days <- 1
  ...
  settings$wake_intensity_mean <- 1; settings$wake_intensity_sd <- 0
  settings$wake_f_mean <- 40; settings$wake_f_sd <- 10 #assume little variation
  settings$wake_duration_mean <- settings$dt; settings$wake_duration_sd <- 0 #assume little variation
  settings$wake_time_mean <- 6.5; settings$wake_time_sd <-0 #people wake up at 7AM on average with a sd of 20 minutes
  ...
  settings$acute_intensity_mean <- 1; settings$acute_intensity_sd <- 0 #intensity of acute stressors
  settings$acute_duration_mean <- settings$dt; settings$acute_duration_sd <- 0 #duration of acute stressors
...
set.seed(n); n=n+1; simpopulation$wake_f_mean_pp <- abs( rnorm(settings$N_people, mean = settings$wake_f_mean, sd = settings$wake_f_sd) ) #can have only positive values
...
for (day in seq(settings$days)){
  ...
  for (sim in seq(settings$N_people)){
    ...
    #frequency
    wake_f <- simpopulation$wake_f_mean_pp[sim]
    ...
    HPA_s <- runif(f,min = settings$work_start, max = settings$work_end)
    HPA_s <- round(HPA_s/dt)

    #create awakening inpuls
    HPA_a <- abs(-rexp(wake_f)+wake_time+simpopulation$wake_delay_mean_pp[sim])
    HPA_a <- round(HPA_a/dt)

    #create a circadian sinus fluctuation
    HPA_c <- circ(circ_intensity, simpopulation$circ_peaktime_pp[sim], N)

    HPA <- 1:N
    for(i in 1:N){HPA[i]<-sum(c(HPA_s,HPA_a)==i)}
    plot(HPA)
