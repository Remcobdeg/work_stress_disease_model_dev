...
paramsets <- list(
  list(#simulation 1
    nightgrowxp = .7#exponential growth factor 
  # ),list(#simulation 2
  #   nightgrowxp = .7#exponential growth factor 
  # ),list(#simulation 2
  #   nightgrowxp = .9#exponential growth factor 
  # ),list(#simulation 2
  #   nightgrowxp = .7#exponential growth factor 
  # ),list(#simulation 3
  #   nightgrowxp = .8#exponential growth factor 
  # ),list(#simulation 4
  #   nightgrowxp = .9#exponential growth factor 
    # ),list(#simulation 2
  #   people = 10000,
...
defaults <- list( #initialize list
  ...
  wakefSD = 20,
  waketime = 7,
  duration = .5, #duration (in hours) of the cortisol awakening response
...
makeHPA <- function(day, simperson, settings,n){
  ...
  #determine timing of the work impulses
  set.seed(n)
  if((day - floor(day/7)*7) %in% settings$dayoff) #for non-working days there is only night time HPA activity
  {set.seed(n); spikes <- abs(-rexp(simperson$wakefM_pp, rate = settings$nightgrowxp) + with(settings, waketime))
  }else{ #for working days, there is also HPA activity during work time
    spikes <- switch(settings$anticipation,
                     NO = c(abs(-rexp(simperson$wakefM_pp, rate = settings$nightgrowxp) + with(settings, waketime)),
                            runif(simperson$stressfM_pp*wordayfactor,min = settings$worktime[[1]], max = settings$worktime[[2]])),
                     YES = c(abs(-rexp(simperson$wakefM_pp+simperson$stressfM_pp*wordayfactor, rate = settings$nightgrowxp) + with(settings, waketime)),
                             runif(simperson$stressfM_pp*wordayfactor,min = settings$worktime[[1]], max = settings$worktime[[2]])))
  }
  spikes <- ceiling(spikes*settings$fs) #corresponding sample moments of the spikes (and prevent spikes at time point 0)
  
  #give work stressors a duration
  burst <- function(spike, duration, fs){
    oneburst <- rep(0, 24*fs)
    oneburst[spike:(spike+duration*fs)] <- 1
    return(oneburst)
  }
  
  allbursts <- ldply(spikes, function(spike) burst(spike, settings$duration, settings$fs))
  HPA <- colSums(allbursts)
  if(length(HPA)==0){HPA <- rep(0, 24*settings$fs)}
...
if(length(datadir)>1){  
  ...
  render("makeprofiles2.Rmd",output_file = paste0(compdir,".html"),params = list(datadir = datadir, paramsets=paramsets))
} else {render("makeprofiles2.Rmd",output_file = paste0(datadir,".html"),params = list(datadir = datadir, paramsets=paramsets))}
