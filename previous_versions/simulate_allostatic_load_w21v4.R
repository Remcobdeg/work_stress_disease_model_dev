...
paramsets <- data.frame(wakefM=rep(15,times=6,each=1),wakefSD=rep(c(30,60,90),times=2,each=1),stressfM=rep(60,times=6,each=1),stressfSD=rep(c(60,120,180),times=1,each=2))
#paramsets <- data.frame(wakefM=rep(c(15,15,30),times=3,each=1),wakefSD=rep(c(10,20,30),times=3,each=1),stressfM=rep(c(10,20,30),times=1,each=3),stressfSD=rep(c(10,20,30),times=1,each=3))
...
if(length(datadir)>1){  
  ...
  render("makegraphscomp.Rmd",output_file = paste0(compdir,".html"),params = list(datadir = datadir, paramsets=paramsets))}
