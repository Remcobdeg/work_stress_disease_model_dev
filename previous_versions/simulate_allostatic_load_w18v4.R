...
if (load_from_file == F){
  ...
  ## simulation settings
  settings$fs <- fs <- 480 #samples per hour
  ...
  settings$wakefSD <- 8
  settings$waketime <- 6
  settings$nightgrow <- c("lin","exp")[2]
  settings$nightgrowxp <- 8 #exponential growth factor, only relevant if exponential growth is chosen
...
makepHPA <- function(settings,fsleep,fwork){
  ...
  if(settings$nightgrow=="lin"){pHPAo <- rep(0,settings$N); pHPAo[night] <- night/sum(night)*fsleep}
  if(settings$nightgrow=="exp"){pHPAo <- rep(0,settings$N); pHPAo[night] <- exp(night/length(night)*settings$nightgrowxp)/sum(exp(night/length(night)*settings$nightgrowxp))*fsleep}
  ...
  if(settings$nightgrow=="lin"){pHPAw <- rep(0,settings$N); pHPAw[night] <- night/sum(night)*fsleep; pHPAw[work] <- 1/length(work)*fwork}
  if(settings$nightgrow=="exp"){pHPAw <- rep(0,settings$N); pHPAw[night] <- exp(night/length(night)*settings$nightgrowxp)/sum(exp(night/length(night)*settings$nightgrowxp))*fsleep; pHPAw[work] <- 1/length(work)*fwork}
