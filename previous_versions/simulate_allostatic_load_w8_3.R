...
#seed <- data.frame(seed = sample(1:99999, 99999, replace = F)); write.csv(seed, 'seeds.csv'); seed <- seed$seed
seed <- read.csv('seeds.csv')[,2]
n <- 1 #seed counter
...
set.seed(seed[n]); n=n+1; simpopulation$halftime_pp <- abs( rnorm(settings$N_people, mean = settings$halftime_mean, sd = settings$halftime_sd) )
set.seed(seed[n]); n=n+1; simpopulation$buffer_capacity_pp <- abs( rnorm(settings$N_people, mean = settings$buffer_capacity_mean, sd = settings$buffer_capacity_sd) ) 
set.seed(seed[n]); n=n+1; simpopulation$r_recovery_pp <- abs( rnorm(settings$N_people, mean = settings$r_recovery_mean, sd = settings$r_recovery_sd) ) 


##awakening impulse
#indivual mean & within subject variation (same as 'intra-individual variation')
set.seed(seed[n]); n=n+1; simpopulation$wake_delay_mean_pp <- abs( rnorm(settings$N_people, mean = settings$wake_delay_mean, sd = settings$wake_delay_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$wake_delay_sd_pp <- simpopulation$wake_delay_mean_pp * settings$multiplier_wake_delay_sd_pp 
set.seed(seed[n]); n=n+1; simpopulation$wake_intensity_mean_pp <- abs( rnorm(settings$N_people, mean = settings$wake_intensity_mean, sd = settings$wake_intensity_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$wake_intensity_sd_pp <- simpopulation$wake_intensity_mean_pp * settings$multiplier_wake_intensity_sd_pp 
set.seed(seed[n]); n=n+1; simpopulation$wake_duration_mean_pp <- abs( rnorm(settings$N_people, mean = settings$wake_duration_mean, sd = settings$wake_duration_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$wake_duration_sd_pp <- simpopulation$wake_duration_mean_pp * settings$multiplier_wake_duration_sd_pp 
set.seed(seed[n]); n=n+1; simpopulation$wake_time_mean_pp <- abs( rnorm(settings$N_people, mean = settings$wake_time_mean, sd = settings$wake_time_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$wake_time_sd_pp <- settings$wake_time_sd_pp #already defined above, without using a multiplier

#circadian pulse
set.seed(seed[n]); n=n+1; simpopulation$circ_peaktime_mean_pp <-rnorm(settings$N_people, mean = settings$circ_peaktime_mean, sd = settings$circ_peaktime_sd)  
set.seed(seed[n]); n=n+1; simpopulation$circ_intensity_mean_pp <- abs( rnorm(settings$N_people, mean = settings$circ_intensity_mean, sd = settings$circ_intensity_sd) )  
set.seed(seed[n]); n=n+1; simpopulation$circ_intensity_sd_pp <- simpopulation$circ_intensity_mean_pp * settings$multiplier_circ_intensity_sd_pp 

##acute stressors
#indivual mean
set.seed(seed[n]); n=n+1; simpopulation$acute_f_mean_pp <- abs( rnorm(settings$N_people, mean = settings$acute_f_mean, sd = settings$acute_f_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$acute_f_sd_pp <- simpopulation$acute_f_mean_pp * settings$multiplier_acute_f_sd_pp
set.seed(seed[n]); n=n+1; simpopulation$acute_intensity_mean_pp <-  abs( rnorm(settings$N_people, mean = settings$acute_intensity_mean, sd = settings$acute_intensity_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$acute_intensity_sd_pp <- simpopulation$acute_intensity_mean_pp * settings$multiplier_acute_intensity_sd_pp
set.seed(seed[n]); n=n+1; simpopulation$acute_duration_mean_pp <-  abs( rnorm(settings$N_people, mean = settings$acute_duration_mean, sd = settings$acute_duration_sd) ) #can have only positive values
set.seed(seed[n]); n=n+1; simpopulation$acute_duration_sd_pp <- simpopulation$acute_duration_mean_pp * settings$multiplier_acute_duration_sd_pp
...
for (sim in seq(settings$N_people)){
  ...
  for (day in seq(settings$days)){
    ...
    #define daily characteristics (if they vary)
    ...
    if (settings$multiplier_acute_f_sd_pp != 0){ 
      ...
      data.day$acute_f[(j-1)*settings$days+k] <- f
    } else {
    ...
    if (settings$multiplier_wake_intensity_sd_pp != 0){ 
      ...
      data.day$wake_intensity[(j-1)*settings$days+k] <- wake_intensity
    } else {wake_intensity <- simpopulation$wake_intensity_mean_pp}
    ...
    if (settings$multiplier_wake_duration_sd_pp != 0){ 
      ...
      data.day$wake_duration[(j-1)*settings$days+k] <- wake_duration
    } else {wake_duration <- simpopulation$wake_duration_mean_pp}
    
    #wake time
    if (settings$multiplier_wake_time_sd_pp != 0){ 
      set.seed(seed[n]); n=n+1; 
      wake_time <- abs(rnorm(1, mean = simpopulation$wake_time_mean_pp[sim], 
                                 sd = simpopulation$wake_time_sd_pp[sim]))
      data.day$wake_time[(j-1)*settings$days+k] <- wake_time
    } else {wake_time <- simpopulation$wake_time_mean_pp}
    ...
    if (settings$multiplier_wake_delay_sd_pp != 0){ 
      ...
      data.day$wake_start[(j-1)*settings$days+k] <- wake_start
    } else {wake_start <- wake_time - simpopulation$wake_delay_mean_pp}
    ...
    if (settings$multiplier_circ_intensity_sd_pp != 0){ 
      ...
      data.day$circ_intensity[(j-1)*settings$days+k] <- circ_intensity
    } else {circ_intensity <- simpopulation$circ_intensity_mean_pp}
    ...
    if (f>0){
      HPA_s <- matrix(NA,24/settings$dt,f) #initialize a column for each stressor, so they can be added later
      #create each individual stressor
      for (i in seq(f)){
        

        HPA_s[,i] <- blockburst(intensity = acute_intensity[i], duration = acute_duration[i], start = acute_start[i], N = 24/settings$dt, dt = settings$dt)
      }
      HPA_s <- apply(HPA_s,1,max) #if stress impulses overlap, the strongest impulse is used. They are not summed
    } else{HPA_s <- rep(0,24/settings$dt)} #if f <= 0 it produces a row of zeros
   
    
    #create awakening inpuls
    HPA_a <- blockburst(intensity = wake_intensity, duration = wake_duration, start = wake_start, N = 24/settings$dt, dt = settings$dt)

  
    #create a circadian sinus fluctuation
    HPA_c <- blockburst(intensity = circ_intensity, duration = circ_duration, start = circ_start, N = 24/settings$dt, dt = settings$dt)
    ...
    #intialize
    data <- data.frame(
      simperson=simpopulation$simperson[sim],
      time=(1:(24/settings$dt))*settings$dt,
      HPA=HPA_s + HPA_a + HPA_c,
      cortisol=as.numeric(c(rep(NA,24/settings$dt))),
      tension=as.numeric(c(rep(NA,24/settings$dt))),
      Aload=as.numeric(c(rep(NA,24/settings$dt))),
    ...
    data$cortisol <- CTA(cortisol_0 = data.day$end_cortisol, 
                         tension_0 = data.day$end_tension, 
                         Aload_0 = data.day$end_Aload, 
                         HPA = data$HPA, 
                         dt = settings$dt, 
    ...
    data$tension <- CTA(cortisol_0 = data.day$end_cortisol, 
                        tension_0 = data.day$end_tension, 
                        Aload_0 = data.day$end_Aload, 
                        HPA = data$HPA, 
                        dt = settings$dt, 
    ...
    data$Aload <- CTA(cortisol_0 = data.day$end_cortisol, 
                      tension_0 = data.day$end_tension, 
                      Aload_0 = data.day$end_Aload, 
                      HPA = data$HPA, 
                      dt = settings$dt, 
    ...
    data.day$end_cortisol <- data$cortisol[N]
    data.day$end_tension <- data$tension[N]
    data.day$end_Aload <- data$Aload[N]
    
    
    # ########### TEMP ################
    # plot(ts(data$cortisol))
    # plot(ts(data$Aload))
    # #################################
}
