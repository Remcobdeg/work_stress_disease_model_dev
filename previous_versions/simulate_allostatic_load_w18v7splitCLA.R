...
if (load_from_file == F){
  ...
  settings$rLSD <- 0
  settings$thAM <- 5 #threshold of allostatic load causing disease
  settings$thASD <- 0
...
getC <- function(HPA, fs, halftime, integration = c("Euler", "RK4")[1]){
...
#### 3.1 CALCULATE CORTISOL & ALLOSTATIC LOAD ####
getLA <- function(C, fs, thL, rL, integration = c("Euler", "RK4")[1]){
  N <- length(C)
  L <- A <- as.numeric(c(0, rep(NA,N-1))) #initialize
...
getscale <- function(settings, simperson, n){   
  ...
  data <- CTA(HPA[1:(with(settings, (waketime+CAR)*fs))], settings$fs, simperson$halftime_pp, simperson$thL_pp, simperson$rL_pp, settings$integration)$C
...
#per person 
settings$scalec<-mean(ldply(seq(settings$people), function(id) getscale(settings = settings, simperson = simpopulation[id,], n = n+id))[,1])*8
