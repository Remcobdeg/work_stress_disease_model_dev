...
if (load_from_file == F){
  ...
  settings <- list() #initialize list
  
  ## simulation settings
  settings$fs <- fs <- 10 #samples per hour
  ...
  settings$wakefSD <- 20
  settings$waketime <- 6
  settings$nightgrowxp <- 1#exponential growth factor 
  
  #work stressors
  settings$stressfM <- 20; 
  settings$stressfSD <- 13 #frequency of stress stressors
  settings$worktime <- c(8,17)
  settings$anticipation <- c("YES","NO")[1]
  
  ## parameters that determine how a cortisol profile translates into allostatic load and sickness
  settings$scaleC <- NA #set to NA to have to code determine the scaling factor
  settings$thLM <- 20 #threshold of dynamic load becoming allostatic load 
  settings$thLSD <- 0
  settings$rLM <- .4 #rate at which the body recovers form the dynamic load
  settings$rLSD <- 0
...
simlog$recovery <- mean(t(as.data.frame(sapply(seq(settings$people), function(id) dosim(settings = settings, simperson = simpopulation[id,], n = n+id))))[,1])
