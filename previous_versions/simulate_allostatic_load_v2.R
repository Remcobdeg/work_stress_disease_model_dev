...
if (settings$N_people >= 5){
  ...
  ggplot(subset(data, ((sim_Person %in% psample) & (time <= 48))), aes(x=time, y=HPA)) +
    geom_line() + facet_wrap( ~ sim_Person, ncol=1) + 
  ...
  ggplot(subset(data, ((sim_Person %in% psample) & (time <= 48))), aes(x=time, y=cortisol)) +
    geom_line() + facet_wrap( ~ sim_Person, ncol=1) + 
