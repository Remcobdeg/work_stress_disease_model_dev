...
paramsets <- list(
  list(#simulation 1
    thLM = 10, #threshold of dynamic load becoming allostatic load 
    rLM = .3, #rate at which the body recovers form the dynamic load
    thAM = 400 #threshold of allostatic load causing disease
  ),list(#simulation 2
    thLM = 20, #threshold of dynamic load becoming allostatic load 
    rLM = .3, #rate at which the body recovers form the dynamic load
    thAM = 400 #threshold of allostatic load causing disease
  ),list(#simulation 2
    thLM = 30, #threshold of dynamic load becoming allostatic load 
    rLM = .3, #rate at which the body recovers form the dynamic load
    thAM = 400 #threshold of allostatic load causing disease
  ),list(#simulation 2
    thLM = 10, #threshold of dynamic load becoming allostatic load 
    ...
    thLM = 10, #threshold of dynamic load becoming allostatic load 
    rLM = .9, #rate at which the body recovers form the dynamic load
    thAM = 400 #threshold of allostatic load causing disease
  ),list(#simulation 2
    thLM = 20, #threshold of dynamic load becoming allostatic load 
    rLM = .9, #rate at which the body recovers form the dynamic load
    ...
    thLM = 10, #threshold of dynamic load becoming allostatic load 
    rLM = .3, #rate at which the body recovers form the dynamic load
    thAM = 100 #threshold of allostatic load causing disease
  ),list(#simulation 2
    thLM = 20, #threshold of dynamic load becoming allostatic load 
    rLM = .3, #rate at which the body recovers form the dynamic load
    thAM = 100 #threshold of allostatic load causing disease
...
defaults <- list( #initialize list
    ...
    #work stressors
    distribution = c("gamma","uniform")[2],
    stressfM = 6, #per 8h, only relevant for gamma
    dshape.stress = 3, #only relevant for gamma
    stressunif = c(1,50), #only relevant for uniform distribution
    worktime = c(9,17),
    anticipation = c("YES","NO")[2],
    dayoff = c(0,6), #from 0-6: 'Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'

    ## physiological parameters
    kHPA = 2.095968, #set to NA to have to code determine the scaling factor
...
exesim <- function(paramset,defaults,subDir){
  ...
  simpopulation$stressfM_pp <- switch(settings$distribution, 
                                      gamma = rgamma(settings$people, shape = settings$dshape.stress, rate = settings$dshape.stress/settings$stressfM), 
...
CTA <- function(HPA, fs, halftime, thL, rL, integration = c("Euler", "RK4")[1], kHPA=1){
  ...
  if(!is.na(kHPA)){#only if the scaling factor is determined will the calculation of L and A continue, otherwise values of C are first use to determine this factor
    C<-C*kHPA #scale cortisol by the predetermined scaling factor
    switch(integration,
           "Euler" =    # Euler numerical integration
...
makeHPA <- function(day, fwake, fstress, settings,n){
  ...
  {set.seed(n); spikes <- abs(-rexp(fwake, rate = settings$nightgrowxp) + with(settings, waketime + CAR))
  }else{ #for working days, there is also HPA activity during work time
    spikes <- switch(settings$anticipation,
                     NO = c(abs(-rexp(fwake, rate = settings$nightgrowxp) + with(settings, waketime + CAR)),
                            runif(fstress*workdayfactor,min = settings$worktime[[1]], max = settings$worktime[[2]])),
                     YES = c(abs(-rexp(fwake+fstress*workdayfactor, rate = settings$nightgrowxp) + with(settings, waketime + CAR)),
                             runif(fstress*workdayfactor,min = settings$worktime[[1]], max = settings$worktime[[2]])))
  }
...
dosim <- function(settings, simperson, n, subDir){   
  ...
  data <- CTA(HPA, settings$fs, simperson$halftime_pp, simperson$thL_pp, simperson$rL_pp, settings$integration, settings$kHPA)
  ...
  write.csv(data.frame(simperson=rep(simperson$simperson,length(savedays)),days=savedays,stressfM=rep(simperson$stressfM_pp,length(savedays)),Aload=data$A[savedays*settings$fs*24],Sick=data$A[savedays*settings$fs*24]>simperson$thA_pp)
...
getscale <- function(settings, simperson, n){   
  ...
  #create a vector with probability values that HPA activity will occur at any possible sample
  HPA <- makeHPA(day=1, fwake[day], fstress[day], settings,n=n+simperson$simperson)
  ...
  data <- CTA(HPA[1:(with(settings, (waketime+CAR)*fs)+1)], settings$fs, simperson$halftime_pp, simperson$thL_pp, simperson$rL_pp, settings$integration, settings$kHPA)
  #data <- CTA(HPA, settings$fs, simperson$halftime_pp, simperson$thL_pp, simperson$rL_pp, settings$integration, settings$kHPA)
...
defaults <- list( #initialize list
  ...
  ## physiological parameters
  kHPA = 2.193871, #set to NA to have to code determine the scaling factor
  halftimeM = 80/60, #half time of cortisol is 80 minutes or 4/3hour
  halftimeSD = 8/60, #between person variation
  thLM = 20, #threshold of dynamic load becoming allostatic load 
  thLSD = 0,
