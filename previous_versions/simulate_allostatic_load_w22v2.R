...
paramsets <- list(
  list(#simulation 1
    ...
    stressfM = 20, #per 8h
    worktime = c(9,(47/3)), #until 15:40
    anticipation = c("YES","NO")[1],
    dayoff = c(6) #from 0-6: 'Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'
  ),list(#simulation 1
    stressfM = 20, #per 8h
    worktime = c(9,(47/3)), #until 15:40
    anticipation = c("YES","NO")[1],
...
defaults <- list( #initialize list
    ...
    kHPA = 7.357749, #set to NA to have to code determine the scaling factor
...
getscale <- function(settings, simperson, n){   
  ...
  data <- CTA(HPA[1:(with(settings, (waketime+CAR)*fs))], settings$fs, simperson$halftime_pp, simperson$thL_pp, simperson$rL_pp, settings$integration, settings$kHPA)
