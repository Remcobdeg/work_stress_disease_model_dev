...
CTA <- function(HPA, fs, halftime, thL, rL, integration = c("Euler", "RK4")[1]){
  ...
  switch(integration,
         "Euler" =    # Euler numerical integration
           mclapply(seq_along(C), function(t) C[[t+1]] <<- Euler(Y1 = C[t], func = lingrowth, param = c(r, HPA[t]), h = 1/fs)),
         "RK4" =   # Runge-Kutta 4th Order Integration
           mclapply(seq_along(C), function(t) C[[t+1]] <<- RK4(Y1 = C[t], func = lingrowth, param = c(r, HPA[t]), h = 1/fs)))
  switch(integration,
         "Euler" =    # Euler numerical integration
           mclapply(seq_along(L), function(t) L[[t+1]] <<- Euler(Y1 = L[t], func = lingrowth, param = c(-rL, C[t]), h = 1/fs)),
         "RK4" =   # Runge-Kutta 4th Order Integration
           mclapply(seq_along(L), function(t) L[[t+1]] <<- RK4(Y1 = L[t], func = lingrowth, param = c(-rL, C[t]), h = 1/fs)))
  mclapply(seq_along(A), function(t) A[[t+1]] <<- ifelse(L[t] > thL, 
                                                       A[t] + (L[t] - thL)*1/fs, 
                                                       A[t]))
...
bindfiles = function(mypath){
  ...
  datalist = mclapply(filenames, function(x){read.csv(file=x,header=T)}) #read the files into a list of data frames
  write.csv(
