...
defaults <- list( #initialize list
  ...
  chisqdf = 3,
  stressfM = 3, #per 8h
  stressfSD = 2, #frequency of stress stressors
  stressunif = c(0,180),
  worktime = c(9,17),
  anticipation = c("YES","NO")[1],
...
makeHPA <- function(day, simperson, settings,n){
  ...
  # }
  if((day - floor(day/7)*7) %in% settings$dayoff) #for non-working days there is only night time HPA activity
  {set.seed(n); spikes <- sample( #sample from a...
    unlist(sapply(seq(with(settings, waketime + CAR)*settings$fs), function(i) rep(i,times = i))), #...linear increasing probability...
    round(simperson$wakefM_pp), replace = T) #...taking wakefM_pp samples

  }else{ #for working days, there is also HPA activity during work time
    spikes <- switch(settings$anticipation,
                     NO = c(
                       sample( #sample from a...
                         unlist(sapply(seq(with(settings, waketime + CAR)*settings$fs), function(i) rep(i,times = i))), #...linear increasing probability...
                         round(simperson$wakefM_pp, replace = T)), #...taking wakefM_pp samples
                       sample(c((settings$worktime[[1]]*settings$fs):(settings$worktime[[2]]*settings$fs)), 
                                round(simperson$stressfM_pp*wordayfactor), replace = T)),
          
                     YES = c(
                       sample( #sample from a...
                         unlist(sapply(seq(with(settings, waketime + CAR)*settings$fs), function(i) rep(i,times = i))), #...linear increasing probability...
                         round(simperson$wakefM_pp+simperson$stressfM_pp*wordayfactor), replace = T), #...taking wakefM_pp samples
                       sample(c((settings$worktime[[1]]*settings$fs):(settings$worktime[[2]]*settings$fs)), 
                              round(simperson$stressfM_pp*wordayfactor), replace = T)))
  }
