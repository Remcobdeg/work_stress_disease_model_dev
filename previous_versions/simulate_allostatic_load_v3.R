...
ALT <- function(HPA, dt, halftime, buffer_capacity, r_recovery){
  ...
  for (t in 1:(N-1)){
    ...
    Dload[t+1] <- Dload[t] * r_recovery + C[t] * dt 
...
settings$r_recovery_mean <- .7 #rate at which the buffer restores
...
settings$wake_intensity_mean <- 15; settings$wake_intensity_sd <- 0
...
#acute stressors
settings$acute_f_mean <- 5; settings$acute_f_sd <- 2 #frequency of acute stressors
settings$acute_intensity_mean <- 6; settings$acute_intensity_sd <- 0 #intensity of acute stressors
settings$acute_duration_mean <- 1/20; settings$acute_duration_sd <- 0 #duration of acute stressors
...
for (j in seq(settings$N_people)){
  ...
  data$Dload[interval] <- ALT(HPA = data$HPA[interval], dt = settings$dt, halftime = simpopulation$halftime_pp[j], buffer_capacity = simpopulation$buffer_capacity_pp[j], r_recovery = simpopulation$r_recovery_pp[j])$Dload
  data$Aload[interval] <- ALT(HPA = data$HPA[interval], dt = settings$dt, halftime = simpopulation$halftime_pp[j], buffer_capacity = simpopulation$buffer_capacity_pp[j], r_recovery = simpopulation$r_recovery_pp[j])$Aload

  ########### TEMP ################
  plot(ts(data$cortisol[interval]))
  plot(ts(data$Dload[interval]))
  #################################
}
