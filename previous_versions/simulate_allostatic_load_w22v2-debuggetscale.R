...
paramsets <- list(
  list(#simulation 1
    ...
    worktime = c(9,19), #4 days
    dayoff = c(0,5,6), #from 0-6: 'Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'
    rLM = .8 #rate at which the body recovers form the dynamic load
  ),list(#simulation 2
    worktime = c(9,9+40/3), #3days
    dayoff = c(0,3,5,6), #from 0-6: 'Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'
    rLM = .8 #rate at which the body recovers form the dynamic load
...
defaults <- list( #initialize list
    ...
    kHPA = 0.3659576, #set to NA to have to code determine the scaling factor
...
exesim <- function(paramset,defaults,subDir){
  ...
  simpopulation$wakefM_pp <- switch(settings$distribution, 
                                    population = abs( rnorm(settings$people, mean = settings$wakefM, sd = settings$wakefSD)), #can have only positive values
                                    uniform = settings$wakefM/(settings$chisqdf-1)*rchisq(settings$people, df=settings$chisqdf),
                                    chisq = settings$wakefM/(settings$chisqdf-1)*rchisq(settings$people, df=settings$chisqdf))
  ...
  simpopulation$stressfM_pp <- switch(settings$distribution, 
                                      ...
                                      chisq = settings$stressfM/(settings$chisqdf-1)*rchisq(settings$people, df=settings$chisqdf))
...
dosim <- function(settings, simperson, n, subDir){   
  ...
  recovery <- sum(data$L[seq(settings$days)*settings$N] < rep(max(data$L)/20,settings$days))/settings$days
