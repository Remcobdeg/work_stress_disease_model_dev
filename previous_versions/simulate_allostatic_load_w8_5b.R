...
CTA <- function(HPA_0, cortisol_0, tension_0, Aload_0, HPA, dt, 
  ...
  #initiate and add start values
  N <- length(HPA)
  HPA <- c(HPA_0, HPA)
  cortisol <- as.numeric(c(cortisol_0, rep(NA,N-1)))
  tension <- as.numeric(c(tension_0, rep(NA,N-1))) #initialize
  Aload <- as.numeric(c(Aload_0, rep(NA,N-1))) #initialize
...
data.day$wake_delay <- with(settings, rep(NA, days*N_people))
data.day$wake_intensity <- with(settings, rep(NA, days*N_people))
data.day$wake_duration <- with(settings, rep(NA, days*N_people))
data.day$circ_intensity <- with(settings, rep(NA, days*N_people))
data.day$acute_f <- with(settings, rep(NA, days*N_people))
...
for (sim in seq(settings$N_people)){
  ...
  for (day in seq(settings$days)){
    ...
    data.day$acute_f[(sim-1)*settings$days+day] <- f
    ...
    #intensity
      ...
      data.day$wake_intensity[(sim-1)*settings$days+day] <- wake_intensity
    ...
    #duration
      ...
      data.day$wake_duration[(sim-1)*settings$days+day] <- wake_duration
    ...
    data.day$wake_time[(sim-1)*settings$days+day] <- wake_time
    ...
    #start of the awakening impulse
      ...
      data.day$wake_start[(sim-1)*settings$days+day] <- wake_start
    ...
    #intensity
      ...
      data.day$circ_intensity[(sim-1)*settings$days+day] <- circ_intensity
    ...
    if (day == 1){
      ...
      HPA_0 <- data.day$end_HPA[day-1]; 
      cortisol_0 <- data.day$end_cortisol[day-1];
      tension_0 <- data.day$end_tension[day-1];
      Aload_0 <- data.day$end_Aload[day-1]}
    dt <- settings$dt 
    ...
    data.day$end_HPA[(sim-1)*settings$days+day] <- data$HPA[N]    
    data.day$end_cortisol[(sim-1)*settings$days+day] <- data$cortisol[N]
    data.day$end_tension[(sim-1)*settings$days+day] <- data$tension[N]
    data.day$end_Aload[(sim-1)*settings$days+day] <- data$Aload[N]
    data.day$sick[(sim-1)*settings$days+day] <- data$Aload[N]>simpopulation$tolerance_pp[sim]
...
ggplot(subset(data.day, day %in% plotdays), aes(x=acute_f, y=end_Aload)) +
