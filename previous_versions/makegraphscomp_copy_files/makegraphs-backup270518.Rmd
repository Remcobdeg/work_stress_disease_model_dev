...
settingsfortable$scaleC<-paste0(round(settingsfortable$scaleC,digits = 3))
...
names(settingsfortable) <-  c("sampling frequency [/h]", "days", "people", "integration method", "off days","cortisol halftime (mean) [h]",   
"cortisol halftime (sd) [h]","CAR [h]",'nighttime HPA-axis impulses (mean)','nighttime HPA-axis impulses (sd)',"time of awakening","factor of exponential growth of night HPA-axis activity",'worktime HPA-axis impulses (mean)','worktime HPA-axis impulses (sd)',"worktime","anticipation of the day implemented","HPA scaling factor","threshold allostatic load (mean)","threshold allostatic load (sd)", "recovery factor dynamic load (mean)","recovery factor dynamic load (sd)","disease threshold (mean)","disease threshold (sd)","saved days","samples/day","sampled people")

if(length(datadir)>1){settingsfortable <- settingsfortable[-which(names(settings) %in% names(paramsets))]} #remove parameters that are varied between simulations

kable(t(as.data.frame(settingsfortable) ))
```
...
simlogs[,c(1:3)]<-paste(round(as.numeric(simlogs[,c(1:3)]), digits = 3),'min')
kable(simlogs)
```
...
if(length(datadir)>1){#only if it concerns multiple simulations
  ...
  getodds2 <- function(data){
    if(length(unique(data$sick))<2 | length(unique(data$sim.nr))<2) #if all instances are either TRUE of FALSE
    {OR<-upper<-lower<-1
    } else {
...
aloadplot <- function(ddir){
  ...
  getsick <- unique(daydata$simperson[which(daydata$Sick==T)])
  getsick <- getsick[1:ifelse(length(getsick>100),100,length(getsick>100))]
  healthy <- unique(daydata$simperson)[-which(unique(daydata$simperson) %in% getsick)]
  healthy <- healthy[1:ifelse(length(healthy>100),100,length(healthy>100))]
...
print('Simulation time:')
      if(exists("totalsimtime")){totalsimtime}
print('Rendering time:')
  difftime(Sys.time(), start.time, unit="mins")
print('Total time:')
ifelse(exists("totalsimtime"),totalsimtime,0) + difftime(Sys.time(), start.time, unit="mins")
