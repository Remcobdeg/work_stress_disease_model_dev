...
if(sum(c("wakefM","wakefSD","stressfM","stressfSD") %in% names(paramsets[[1]]))>0){
...
if(length(datadir)>1 & table(c('rLM','thLM','thAM') %in% names(paramsets[[1]]))[["TRUE"]]==3){
  ...
  odds_all$rL<-as.factor(odds_all$rL)
  ...
  ggplot(odds_all, aes(y= OR, x = day, shape = A_D)) +
    ...
    scale_shape_discrete(name=expression('A'['D'])) +
    labs(title = expression('Relative disease risk - split by value of L'['A']*' (horizontal) and r'['L']*' (vertical)'), x = "Day", y = "Relative disease risk") +
    theme_bw()
...
ggplot(subset(daydataall, days == max(days)), aes(x=stressfM, y=Aload+1)) +
  geom_point() + geom_smooth(method="auto", se=F, colour="grey") +
  facet_grid(rL ~ L_A) + 
  coord_trans(y="log2") +
