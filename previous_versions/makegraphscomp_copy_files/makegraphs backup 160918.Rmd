...
if(length(datadir)>1 & sum(c('rho.E','eps.A','eps.D') %in% names(paramsets[[1]]))==3){
  ...
  odds_all$rho.E<-as.factor(odds_all$rho.E) # label = c(paste(expression(rho[E]*' = .3')),paste(expression(rho[E]*' = .6')),paste(expression(rho[E]*' = .9'))))
...
ggplot(odds_all, aes(y= OR, x = day, shape = eps.D)) +
  geom_point() +
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.2) +
  facet_grid(eps.A ~ rho.E, labeller = label_bquote(rows = rho[E] == .(.9), cols = epsilon['A'] == .(30))) +
  geom_hline(yintercept = 1, linetype=2) +
  #coord_flip() +
  coord_trans(y="log10") +
...
if(length(datadir)>1 & sum(c('rho.E','eps.A','eps.D') %in% names(paramsets[[1]]))==3){
  ...
  ggplot(subset(daydataall, days == max(days) & eps.D == unique(eps.D)[1]), aes(x=scale(work.f.mean)[,1], y=Aload)) +
