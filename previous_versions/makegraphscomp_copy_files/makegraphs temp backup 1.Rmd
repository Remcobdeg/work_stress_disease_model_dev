...
if(sum(c("night.f.mean","wakefSD","work.f.mean","stressfSD") %in% names(paramsets[[1]]))>0){
...
makeQgraph <- function(ddir){#first create plot of quantiles since awakening, as in Miller et al.
  ...
  pa<- ggplot(subset(Qdata, time>=settings$waketime), aes(x=time-settings$waketime,y=cortisol,group=quantile)) + 
    ...
    labs(title=paste0("f-wake=",settings$night.f.mean,"(",settings$wakefSD,") ","f-stress=",settings$work.f.mean,"(",settings$stressfSD,")"),x="time") +
  ...
  pt<-ggplot(Qdata, aes(x=time,y=cortisol,group=quantile)) + 
    geom_line(aes(linetype=quantile)) + 
    scale_linetype_manual(values=c("dotted","twodash","solid","twodash","dotted")) +
    labs(title=paste0("f-wake=",settings$night.f.mean,"(",settings$wakefSD,") ","f-stress=",settings$work.f.mean,"(",settings$stressfSD,")")) +
...
if(length(datadir)>1 & sum(c('rho.E','eps.A','eps.D') %in% names(paramsets[[1]]))==3){
  ...
  getodds_all <- function(ddir){
    ...
    odds <- ldply(unique(daydata$days), function(day) getodds(subset(daydata, days==day)))
    odds$rL <- rep(settings$rho.E, nrow(odds))
    odds$L_A <- rep(settings$eps.A, nrow(odds))
    odds$A_D <- rep(settings$eps.D, nrow(odds))
...
#### 5.3 Allostatic load ~ average stressor frequency ####
  ...
  get_aload <- function(ddir){
    ...
    daydata$rL<-as.factor(rep(settings$rho.E,nrow(daydata)))
    daydata$L_A<-as.factor(rep(settings$eps.A,nrow(daydata)))
    daydata$A_D<-as.factor(rep(settings$eps.D,nrow(daydata)))
  ...
  ggplot(subset(daydataall, days == max(days) & A_D == 400), aes(x=scale(work.f.mean)[,1], y=Aload)) +
...
aloadplot <- function(ddir){
  ...
  p <- ggplot(daydata, aes(x=work.f.mean, y=Aload)) +
