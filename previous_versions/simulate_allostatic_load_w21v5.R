...
exesim <- function(paramset,subDir){
  ...
  set.seed(n); n=n+1; simpopulation$wakefM_pp <- abs( rnorm(settings$people, mean = settings$wakefM, sd = settings$wakefSD)-1 )+1 #can have only positive values
...
makeHPA <- function(day, simperson, settings,n){
  ...
  {set.seed(n); spikes <- abs(-rexp(simperson$wakefM_pp, rate = settings$nightgrowxp) + with(settings, waketime + CAR))
    ...
    spikes <- switch(settings$anticipation,
           ...
           YES = c(abs(-rexp(simperson$wakefM_pp+simperson$stressfM_pp, rate = settings$nightgrowxp) + with(settings, waketime + CAR)),
                               runif(simperson$stressfM_pp,min = settings$worktime[[1]], max = settings$worktime[[2]])))
    }
