...
if (load_from_file == F){
  ...
  ## off-work days
  settings$dayoff <- c(0,3,7) #weekday 0 == weekday 7; for computational reasons, always include both
  ...
  settings$thLM <- 5 #threshold of dynamic load becoming allostatic load 
  settings$thLSD <- 0
  settings$rLM <- .1 #rate at which the body recovers form the dynamic load
  settings$rLSD <- 0
  settings$thAM <- 130000 #threshold of allostatic load causing disease
  settings$thASD <- 0
...
makeHPA <- function(day, simperson, settings,n){
  ...
  spikes <- ceil(spikes*settings$fs) #corresponding sample moments of the spikes (and prevent spikes at time point 0)
...
settings$filemergetime <- Sys.time() - start.time
