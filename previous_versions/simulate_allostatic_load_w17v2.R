...
makepHPA <- function(settings,fsleep,fwork){
  ...
  pHPAo <- rep(0,N); pHPAo[night] <- night/sum(night)*fsleep
  ...
  pHPAw <- rep(0,N); pHPAw[night] <- night/sum(night)*fsleep; pHPAw[work] <- 1/length(work)*fwork
  
  #make a vector that combines days of work and non-work, depending of which days are off-days
  pHPA <- stack(as.data.frame(sapply(1:settings$days, function(day) if((day - floor(day/7)*7) %in% settings$daysoff){pHPAo}else{pHPAw})))[1]
  return(pHPA)
}
...
for (sim in seq(people)){
  ...
  pHPA <- makepHPA(settings,fsleep=simpopulation$wakefM_pp[sim],fwork=simpopulation$stressfM_pp[sim])
  
  #create HPA activity by comparing each probability value with a random generated number [0:1]
  set.seed(n); n=n+1; 
  HPA <- (ifelse(runif(N*days)<pHPA,1,0))  
  
  #### 4.4.3 calculate cortisol, L and allostatic load from the HPA activity ####
  
  data <- CTA(HPA, fs, simpopulation$halftime_pp[sim], simpopulation$thL_pp[sim], simpopulation$rL_pp[sim], settings$integration)
...
# if("stressf" %in% colnames(daydata)){
#   daydata$stressfM <- rep(NA, nrow(daydata))
#   for (i in 1:nrow(daydata)){
#     daydata$stressfM[i] <- with(daydata, mean(stressf[which(simperson==simperson[i] & workday == T)]))
#     }
# } else {
#   daydata$stressfM <- rep(round(simpopulation$stressfM_pp),times = settings$days)
# }
# 
