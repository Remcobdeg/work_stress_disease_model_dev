...
if (load_from_file == F){
  ...
  ## simulation settings
  settings$fs <- fs <- 60 #samples per hour
  ...
  settings$nightgrowxp <- 5 #exponential growth factor, only relevant if exponential growth is chosen
...
makepHPA <- function(settings,fsleep,fwork){
  ...
  pHPAo <- rep(0,settings$N); pHPAo[night] <- exp(night/length(night)*settings$nightgrowxp)/sum(exp(night/length(night)*settings$nightgrowxp))*fsleep
  ...
  pHPAw <- rep(0,settings$N); pHPAw[night] <- exp(night/length(night)*settings$nightgrowxp)/sum(exp(night/length(night)*settings$nightgrowxp))*fsleep; pHPAw[work] <- 1/length(work)*fwork
