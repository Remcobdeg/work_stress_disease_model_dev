...
exesim <- function(paramset){
  ...
  simlog$preptime <- Sys.time() - start.time
  ...
  #simlog$recovery <- mean(t(as.data.frame(sapply(seq(settings$people), function(id) dosim(settings = settings, simperson = simpopulation[id,], n = n+id, subDir = subDir))))[,1])
  ...
  simlog$simtime <- Sys.time() - start.time
  ...
  simlog$filemergetime <- Sys.time() - start.time
...
sapply(seq(nrow(paramsets)), function(i) exesim(paramsets[i,]))
