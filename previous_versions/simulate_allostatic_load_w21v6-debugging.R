...
paramsets <- data.frame(wakefM=rep(20,times=4,each=1),wakefSD=rep(c(20,40),times=2,each=1),stressfM=rep(40,times=4,each=1),stressfSD=rep(c(40,60),times=1,each=2))
...
paramsets <- data.frame(wakefM=20,wakefSD=20,stressfM=0,stressfSD=0,scaleC=1.864)
...
exesim <- function(paramset,subDir){
  ...
  if(is.na(settings$scaleC)){settings$scaleC<-9/mean(ldply(seq(settings$people), function(id) getscale(settings = settings, simperson = simpopulation[id,], n = n+id))[,1])}
...
if(length(datadir)>1){  
  ...
  render("makegraphs.Rmd",output_file = paste0(compdir,".html"),params = list(datadir = datadir, paramsets=paramsets, totalsimtime=difftime(Sys.time(), start0, unit="mins")))
  }
